...
function create(content: string): Todo {
  ...
  //array  
const todos: Array<Todo> = [    // Todo[] ou Array<Todo> são a mesma coisa, o segundo se chama diamond operator  
    ...
    fs.writeFileSync(DB_FILE_PATH, JSON.stringify({
        todos,
    }, null, 2));                                          /*além de transformar obj em string, o stringify dá pra configurar o espaçamento
...
function read(): Array<Todo>{
    ...
    const db =  JSON.parse(dbstring || "{}");                       /*JSON.parse(dbstring)carrega o arquivo coo string e converte para um banco de dados 
                                                                    que pode ter qualquer tipo de dado. OUTRA COISA, esse || "{}" vai 
                                                                    servir para que, caso o banco esteja vazio, não retorne erro, 
                                                                    e sim algo vazio de fato */
